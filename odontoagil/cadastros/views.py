...
def editar_servicos(request, servico_id):
    servico = get_object_or_404(Servico, id=servico_id)  # Busca o objeto no banco
    ...
    if request.method == 'POST':
        ...
        servico.codigo_servicos_secundarios = request.POST.get('codigo_servicos_secundarios')
        servico.descricao_servicos_secundarios = request.POST.get('descricao_servicos_secundarios')
        servico.aparelho_apoio = request.POST.get('aparelho_apoio')
        servico.observacoes = request.POST.get('observacoes')

        servico.save()  # Salva as alterações

        return redirect('listar_servicos')  # Redireciona após salvar
...
def cadastrar_servicos(request):
    if request.method == 'POST':
        # Criando uma nova instância do modelo Servico
        servico = Servico(
            ...
            codigo_servicos_secundarios=request.POST.get('codigo_servicos_secundario'),
            descricao_servicos_secundarios=request.POST.get('descricao_servicos_secundario'),
...
def editar_servicos(request, servico_id):
    servico = get_object_or_404(Servico, id=servico_id)  # Obtém o objeto correto

    if request.method == 'POST':
        descricao_servicos = request.POST.get('descricao_servicos', '').strip()

        # Validação para evitar erro de campo obrigatório vazio
        if not descricao_servicos:
            return render(request, 'servicos/editar_servicos.html', {'servico': servico, 'error': 'A descrição é obrigatória'})

        # Atualiza os campos
        servico.data_inclusao = request.POST.get('data_inclusao')
        servico.codigo_servicos = request.POST.get('codigo_servicos')
        servico.material_usado = request.POST.get('material_usado')
        servico.descricao_servicos = descricao_servicos
        servico.codigo_servicos_secundarios = request.POST.get('codigo_servicos_secundarios')
        servico.descricao_servicos_secundarios = request.POST.get('descricao_servicos_secundarios')
        servico.aparelho_apoio = request.POST.get('aparelho_apoio')
        servico.observacoes = request.POST.get('observacoes')
